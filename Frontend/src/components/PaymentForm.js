...
try{
  if(Number.length === 10){
  var data1 = await fetch("http://localhost:80/Order",{
    method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({amount,name, email, Town, Number })
  })
 
  var keys= await fetch("http://localhost:80/key",{
    method: 'GET',
  ...
  const  options = {
    ...
    callback_url: "http://localhost:80/verification",
...
const rzp1 = new window.Razorpay(options);
  ...
  const saveDataToDatabase = async () => {
    ...
    try {
      ...
      const response = await fetch("http://localhost:80/saveDataToDatabase", {
        method: "POST",
        headers: {
